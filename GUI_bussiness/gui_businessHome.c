				...
				case devTypeDef_voltageSensor:{
					...
					if(false == devDriverBussiness_SSMR_devVoltState_get()){ //电压告警
						...
						if(dispFlg_rcd != dispFlg){
							...
							if(dispFlg)sprintf(tempInfomationDisp_volt, "#FF2020 %.02fV#", volatageVal_cur);
							else sprintf(tempInfomationDisp_volt, "   ");
							if(false == devDriverBussiness_solarSysManager_voltageAlarmEn_get()) //如果未打开报警使能，则不进行闪烁
								sprintf(tempInfomationDisp_volt, "#FF2020 %.02fV#", volatageVal_cur);
					...
					else //正常
					{
						if(volatageVal_record != volatageVal_cur){
							...
							memset(tempInfomationDisp_volt, 0, sizeof(char) * 32);
							sprintf(tempInfomationDisp_volt, "#B0E61D %.02fV#", volatageVal_cur);
							lv_label_set_text(labelObj_solarSysManager_voltage, (const char *)tempInfomationDisp_volt);
...
//							lv_obj_set_top(imageObj_solarSysManager_meansureUnit, true);
					...
					if(false == devDriverBussiness_SSMR_devTempState_get()){ //温度告警
						...
						if(dispFlg_rcd != dispFlg){
							...
							if(dispFlg)sprintf(tempInfomationDisp_temp, "#FF2020 %.02fC#", tempratureVal_cur);
							else sprintf(tempInfomationDisp_volt, "   ");
					...
					else
					{
						if(tempratureVal_record != tempratureVal_cur){
							...
							memset(tempInfomationDisp_temp, 0, sizeof(char) * 32);
							sprintf(tempInfomationDisp_temp, "#B0E61D %.02fC#", tempratureVal_cur);
							lv_label_set_text(labelObj_solarSysManager_temprature, (const char *)tempInfomationDisp_temp);
//							lv_obj_set_top(imgObj_solarSysManager_voltage, true);
//							lv_obj_set_top(imgObj_solarSysManager_temprature, true);
//							lv_obj_set_top(imageObj_solarSysManager_meansureUnit, true);
...
static void local_guiHomeBussiness_solarSysManager(lv_obj_t * obj_Parent){
	...
	sprintf(strTemp_dispInfo, "#B0E61D %.02fV#", volatageVal_cur);
	...
	memset(strTemp_dispInfo, 0, sizeof(strTemp_dispInfo));
	sprintf(strTemp_dispInfo, "#B0E61D %.02fC#", tempratureVal_cur);
...
static void lvGuiHome_styleMemoryInitialization(void){
	...
	styleLabel_solarSysManager_valueDisp = (lv_style_t *)os_zalloc(sizeof(lv_style_t));
	styleLabel_solarSysManager_refDisp = (lv_style_t *)os_zalloc(sizeof(lv_style_t)); 
	styleImage_solarSysManager_voltIcon = (lv_style_t *)os_zalloc(sizeof(lv_style_t));  
	styleImage_solarSysManager_tempIcon = (lv_style_t *)os_zalloc(sizeof(lv_style_t));  
	styleImage_eleIcon_largeCurrent = (lv_style_t *)os_zalloc(sizeof(lv_style_t)); 
