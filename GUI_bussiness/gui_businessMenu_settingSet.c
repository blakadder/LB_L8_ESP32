...
void devForceAsMeshChild_opreatConfirm(void){
	...
	lv_mbox_set_text(mboxDevForceAsChild_comfirm, "This operation needs to be restarted to take effect, confirm to continue.");
...
//	}
	...
	lv_obj_align(textSettingA_devStatusRecordIF, lvObjAlign_temp, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 60);
...
//	lv_obj_align(textBtnRef_touchRecalibrationIf, NULL, LV_ALIGN_CENTER, 0, 0);
	...
	lv_obj_align(textSettingA_tempCalibration, textSettingA_screenBrightness, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 60);
...
void mwifiApp_atmosLightColorCfg_boardcastNotice(void){
	...
	if(false == mwifi_is_connected())return; //当前网络不可用
		...
		uint8_t *dataTX_temp = (uint8_t *)os_zalloc(sizeof(uint8_t) * dataTX_len);
