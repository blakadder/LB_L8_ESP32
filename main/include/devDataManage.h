...
#define L8_DEVICE_VERSION_REF_DISCRIPTION		"L8-v1.0.32[202007221015]" //固件说明描述版本号
...
#define DEVICE_DRIVER_DEFINITION				DEVICE_DRIVER_METHOD_BY_SELF_HARDWARE
 ...
 #define DEV_TYPES_PANEL_DEF_SOLAR_SYS_MANAGER	(0x16 + DEVICE_FLASH_PARTITIONS_OFFSET)	//面板类型定义：太阳能系统管理器
#else
...
#define STR_VERSION_REALES_REFERENCE_LAST		" 1.'Homeassistant' supported, enter\n"\
												"   Settings to enable this function.\n\n"\
												" 2.Add device management capabilities,\n"\
												"   enter Settings to enable this\n"\
												"   function.\n\n"\
												" 3.Significantly improved crash\n"\
												"   restart issues.\n\n"\
												" 4.The L8 mesh network is compatible with\n"\
												"   the new device \"Voltage Sensor\".\n\n"

#define STR_VERSION_REALES_REFERENCE_DFT		" 1.The L8switch border light can\n"\
	...
	#define STR_VERSION_REALES_REFERENCE		" 1.When the voltage is too low, \n"\
...
typedef struct struct_devCtrlObjGraphBlockOperationParam{

	lv_obj_t *objBaseBlockSelect_cb;
	LV_OBJ_FREE_NUM_TYPE lv_fNum_objBlockCb;
	uint8_t objBlockSelect_flg:1;
...
stt_solarSysManagerDevList_nvsOpreat *nvsDataOpreation_solarSysManagerDevList_get(void);
